...
export function NotesTab({ recipeId }: NotesTabProps) {
  ...
  const renderNote = ({ item }: { item: RecipeNote }) => (
    <ThemedView style={[styles.noteCard, { backgroundColor: "transparent" }]}>
      <ThemedView style={styles.noteHeader}>
        ...
        <Pressable onPress={() => handleDeleteNote(item.id)} hitSlop={8}>
          <IconSymbol
            name="trash"
            size={16}
            color={textColor}
      ...
      {isAdding && (
        ...
        >
          ...
          <ThemedView style={styles.inputActions}>
            <Pressable onPress={handleCancelAdding} style={styles.inputButton}>
