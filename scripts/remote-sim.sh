...
resolve_build_id() {
  ...
  # Pick the newest build whose artifact has not expired yet. EAS expires
  # simulator artifacts (typically 30 days), and an expired build cannot be
...
usable = []
for b in builds:
    exp = parse(b.get("expirationDate"))
...
if not usable:
    sys.stderr.write(
        "No unexpired 'development-simulator' iOS build found.\n"
        "Build one with:\n"
...
best = usable[0]
sys.stderr.write(
    f"  build {best['id']}  completed {best.get('completedAt')}  "
    f"expires {best.get('expirationDate')}\n"
)
print(best["id"])
PY
}
